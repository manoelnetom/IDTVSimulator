...
import br.edu.ifba.subscriber.SimpleCarouselListener;
import br.edu.ifba.subscriber.carousel.Carousel;
import java.io.File;
...
//            log.debug("current channel is now " + ((Channel) this.channels.get(channel)).getName());
            ...
            Media media = new Media("c:" + File.separator + "vlc.mp4");
            MediaPlayer.getInstance().setMedia(media);

            Carousel carousel = new Carousel(2222);
            carousel.addListener(new SimpleCarouselListener());
            carousel.start();
