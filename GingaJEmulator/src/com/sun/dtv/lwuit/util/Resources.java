...
import com.sun.dtv.lwuit.EncodedImage;
import com.sun.dtv.lwuit.IndexedImage;
import com.sun.dtv.lwuit.Font;
import com.sun.dtv.lwuit.Image;
import com.sun.dtv.lwuit.StaticAnimation;
import com.sun.dtv.lwuit.animations.AnimationObject;
import com.sun.dtv.lwuit.animations.Timeline;
...
public class Resources {
    ...
    Image createImage() throws IOException {
        if(majorVersion == 0 && minorVersion == 0) {
            ...
            System.out.println("Carregando o bitmap da fonte.");
            ...
            switch(type) {
                ...
                case 0xF5: {
                    ...
                    //Currently svg is not supported, so we skip it
    ...
    com.sun.dtv.lwuit.Image createSVG(boolean animated, byte[] data) throws IOException {
        return null;//Image.createSVG(null, animated, data);
    }
    ...
    Image readMultiImage(DataInputStream input, boolean skipAll) throws IOException {
        ...
        int dpi = 60; //Please view lwuit Display class for density value
    ...
    Font loadBitmapFont(DataInputStream input, String id, com.sun.dtv.lwuit.Font font) throws IOException {
    ...
    Hashtable loadTheme(String id, boolean newerVersion) throws IOException {
        ...
        for(int iter = 0 ; iter < size ; iter++) {
            ...
            if(key.endsWith("ackground")) {
                ...
                theme.put(key, new Byte((byte)type));
                ...
                switch(type) {
                    ...
                    case 0xF7:
                        ...
                        theme.put(key, new Object[] {new Integer(input.readInt()), new Integer(input.readInt()),c, c, new Float(1)});
                        break;
                    ...
                    case 0xF8:
                        ...
                        theme.put(key, new Object[] {new Integer(c1),
                            new Integer(c2),
                            new Float(f1),
                            new Float(f2),
        ...
        switch(type) {
            ...
            case 0xff02:
                ...
                if(input.readBoolean()) {
                	...
                	Border b = Border.createLineBorder(input.readByte());
                	input.readInt();
                	return b;
                }
            ...
            case 0xff03:
                ...
                if(input.readBoolean()) {
                    ...
                    Border b = Border.createRoundBorder(input.readByte(), input.readByte());
                    input.readInt();
                    return b;
                }
            ...
            case 0xff04:
                ...
                if(input.readBoolean()) {
                	...
                	Border b = Border.createEtchedLowered(input.readInt(), input.readInt());
                }
